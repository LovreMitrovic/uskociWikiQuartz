...
## [2026-09-08] ingest | Stanojević 1756 Zadar census (Grimani survey)
- Source page: [[Stanojevic-1756-Zadar-Census]] — Gligor Stanojević (ed.), "Попис становништва и земље у околини Задра из 1756. године", *Мешовита грађа* XVI (1987), Београд 1988, стр. 49–126; original ASV, *Provveditori generali in Dalmazia et Albania*, filza 596. Raw artifact `raw/transcriptions/stanojevic-1756-census.md` (markdown tables; the PDF is a scan with **no text layer**, so the transcription is OCR/vision, not `pdftotext`).
- **Persons added: 1,431** — 1,317 resident household heads + 114 named non-resident landholders (Zadar patricians/citizens holding land in the villages). 10,330 souls across 24 villages.
- **Surnames: 681 canonical** — 553 new hubs, 128 existing hubs gained a `## 1756 census bearers` section.
- **Places: 8 new** ([[Rastevic|Rastević]], [[Radasinovac|Radašinovac]], [[Galovac]], [[Rastane|Raštane]], [[Gornja-Jagodnja|Gornja Jagodnja]], [[Donja-Jagodnja|Donja Jagodnja]], [[Gornje-Biljane|Gornje Biljane]], [[Donje-Biljane|Donje Biljane]]), 16 existing gained a 1756 roster, 3 more gained cross-reference notes ([[Jagodnja]], [[Biljane]], and the former `Toreta`, since merged into [[Turanj]]).
- Decisions (user): **full explosion** (one Person page per named head); **non-resident landholders recorded as Person pages too**; the introductory essay (pp. 49–51), absent from the transcription, was **not** OCR'd — skipped at user's instruction.
- Conventions applied:
  - `birth_place` set to the census village on every page, with **`birth_place_inferred: true`** and a Notes bullet stating the field is inferred from residence and no birth record is attested. New CLAUDE.md convention (*Birth place when only residence is attested*), added this session at user's request. Supersedes the [[Razov-Milkovic-Skabrnja-Monografija]] approach of omitting the field; those 70 Škabrnja pages are **not** retrofitted, because that village was resettled after the Morean War — contrary evidence, so the default does not apply there.
  - Both name forms kept: `given_name` + `given_name_variants: [Venetian]`, `surname` + `surname_variants: [Venetian]`, full original name in body prose.
  - `confidence: medium` (554 pages) where the canonical surname already had a hub in the wiki before this ingest, i.e. is independently attested; `confidence: low` (877 pages) where the surname enters the wiki only through this source.
  - Person id collisions: 20 pages carry a numeric suffix (`-2`). Each such page has a Notes line naming the distinguishing detail from the source (*detto*, *quondam*, *Vecchio*, title) and links its twin.
- Transcription defects handled (all flagged on the affected pages and on the source page):
  - **Donje Jagodnje pp. 85–87** opens with a garbled block of 9 households repeating entries that recur later with full land figures — merged to the fuller entry (Plazibat, Vrtača, Toma + Miše Žepina, Mitar/Mijo/Petar Žarković, Pantalija, Kovačević). For *Toma Žepina* (19 vs 10 souls) and *Miše Žepina* (26 vs 22) the copies disagree; both figures recorded in Notes.
  - **Biograd pp. 64–69** split across two headings, largely complementary alphabetical runs; *Zuanne Rudich* and *Marco Rudich* appear in both — merged. Treated as one village.
  - **Pakoštane** *Zorzi Rasin* entered twice on adjacent lines with identical souls and campi — merged.
  - 12 merges in total.
- Name-normalization notes:
  - `Xorich` and `Zorich` both occur **in Korlat**, so they were merged into the single hub [[Zoric|Zorić]] rather than split as Žorić/Zorić; both spellings kept as variants.
  - `Milcovich` → [[Milkovic|Milković]] and `Mircovich` → [[Mirkovic|Mirković]] — corrected against a bad automatic variant match to Milić / Miković.
  - `Pleticossa` → [[Pletikosa]] kept distinct from `Pleticosich` → [[Pletikosic|Pletikosić]].
  - Zadar patrician/citizen surnames left in their Italian form (Rotta, Stratico, Filippi, Adobati, Albinoni, Bonicelli, Detrico, Canagetti, Bianchi, Piovesana, Costantini, Diana, Giusti, Pasini, Pezzi, Pinelli, Scarpa, Ventura, Spingaroli, Morosini, Bortolazzi, Cortese, Maltese, Messinese, Visinoni), matching [[Soppe]], [[Benja]], [[Fanfogna]], [[Vlasto]], [[Posedarski]].
- **Zemunik** enumerated as two communities — *villici* (70 hh, 530 souls) and *Albanesi* (70 hh, 415 souls), the colony settled from the Bar / Šestani region 1726–1733. Both rosters on [[Zemunik]]; each Person page names its community. New Albanian-layer hubs include [[Curkovic|Ćurković]] (15 bearers), [[Duka]], [[Bitri]], [[Pettani]], [[Vladanj]], [[Markov]], [[Jovi]], [[Kalmetic|Kalmetić]], [[Dede]], [[Dedic|Dedić]]. Four *nuovi abitanti* listed inline in a summary row were extracted as Person pages.
- Open questions:
  - **Introductory essay (pp. 49–51) deliberately not transcribed** (user decision) — Stanojević's methodology, his own village count, and his demographic commentary remain unavailable. Would need `tesseract -l srp` (Serbian Cyrillic pack not installed) or a manual read of the scan. Closed, not pending.
  - **[[Sv-Filip-i-Jakov|Sv. Filip i Jakov]] is missing from the 1610 ingest.** The village *is* in the Dundović census — 16 heads, 110 souls, `raw/transcriptions/dundovic-1610-zadar-census.txt` lines 1321–1347 — but the 2026-07-21 ingest skipped it, so there is no 1610 roster and no comparison section could be built. Backfill pending.
  - `Banjevac` (census) filed under the existing [[Banjevci]] — same village assumed, not verified.
  - **20 surname hub files whose Person pages disagree on the canonical spelling** across the two ingests — almost all a missing š in the older form: Bošković, Drašković, Galešić, Jadrešić, Meštrović, Mišulić, Piližarić, Piljušić, Pušić, Radašinović, Rašnjević, Šarić, Šiljanović, Šodić, Vičinović, Višić, Vranešić, Vušković. Two need judgement rather than a blanket fix: **Pećić/Pečić** (genuinely different names, ć vs č) and **Zrinski/Zrinški** (where the *undotted* form is the correct one). Only [[Draskovic|Drašković]]'s hub `canonical` was corrected this session; the rest are reported, not touched.
  - Low-confidence normalizations (surnames attested only here) remain candidates for correction against the Jelić 1608 material and any 18th-c. parish registers.
  - One id collision with a 1610 person: [[Toma-Matkovic-b????-Biograd-na-Moru-2]] (1756) vs [[Toma-Matkovic-b????-Biograd-na-Moru]] (1610) — different men, cross-noted.

## [2026-09-08] note | Post-ingest follow-ups on the 1756 census
Four changes requested after the ingest report:
- **`birth_place` convention changed.** New CLAUDE.md section *Birth place when only residence is attested*: assume birth place = residence place when nothing better is attested, and flag it with `birth_place_inferred: true` plus a Notes bullet. Applied to all 1,431 [[Stanojevic-1756-Zadar-Census]] Person pages. Not retrofitted to the 70 [[Razov-Milkovic-Skabrnja-Monografija]] pages (resettled village = contrary evidence) nor to the [[Dundovic-1610-Zadar-Census]] pages (which already carry an unflagged `birth_place`) — both are candidates for a follow-up pass.
- **Introductory essay pp. 49–51: closed as skipped**, not pending.
- **1610 → 1756 comparison written into the village pages** rather than as a standalone page, per instruction. `## 1610 → 1756: what changed` added to [[Sukosan|Sukošan]], [[Turanj]], [[Pakostane|Pakoštane]], [[Biograd-na-Moru|Biograd na Moru]]: household/soul/surname counts side by side, the surnames in both, the disappearances, the arrivals, the likely same-family pairs split by spelling, and a caveat that the turnover figures are a lower bound.
  - Continuity is low everywhere — Sukošan 12/39, Turanj 5/33, Pakoštane 2/14, Biograd 8/51.
  - Best single find: **[[Vitanovic|Vitanović]]**, 5 households in 1610 Biograd, vanishes as a surname but survives in 1756 as the byname *detto Vitanovich* on three [[Mrsun|Mršun]] households and one [[Zoric|Zorić]] — a family absorbed and demoted to an epithet, invisible if you compare surname lists alone.
  - Flagged as probably-one-family but **not merged**: [[Smolic|Smolić]]~[[Smolic|Smolić]] (Sukošan — the strongest case), [[Nadahnic|Nadahnić/Nadihnić]]~[[Nadinic|Nadinić]], [[Siskic|Siškić]]~[[Sisic|Šišić]], [[Baresic|Barešić]]~[[Barisic|Barišić]], [[Frakasov]]~[[Frakas]], [[Matulj]]~[[Matilj]], [[Veziljevic|Veziljević]]~[[Vesiljevic|Vesiljević]], [[Jelicic|Jeličić]]~[[Jelcic|Jelčić]], [[Pilizaric|Pilizarić]]/[[Pilizaric|Piližarić]]~[[Pelizaric|Pelizarić]], [[Dabanovic|Dabanović]]~[[Dobanovic|Dobanović]], [[Zmajcic|Zmajčić]]~[[Smajcic|Smajčić]], [[Maratov]]~[[Marat]].
  - [[Sv-Filip-i-Jakov|Sv. Filip i Jakov]] could not be compared — see the ingest entry above.
- **The duplicate place page `Toreta` was merged into [[Turanj]]** and deleted: historical names *Torreta* / *Toretta* and the Desnica II Zaravecchia-pertinenza note folded in, sources merged. Confirmed same settlement (*Torrete* / *Toretta de Marco*).
- **Gornja/Donja pairs kept separate** — [[Gornja-Jagodnja|Gornja Jagodnja]] / [[Donja-Jagodnja|Donja Jagodnja]] and [[Gornje-Biljane|Gornje Biljane]] / [[Donje-Biljane|Donje Biljane]] are distinct villages, not duplicates. [[Jagodnja]] and [[Biljane]] stay as collective entries carrying the pre-1756 material that isn't resolved to upper/lower.

## [2026-09-08] lint | Surname canonical-spelling reconciliation across ingests
Cleanup of the 20 surname hubs whose Person pages disagreed on the canonical spelling between the [[Dundovic-1610-Zadar-Census]] and [[Stanojevic-1756-Zadar-Census]] ingests (surfaced by the 1610→1756 village comparisons).
- **18 reconciled** — in every case the older form had dropped a diacritic; 39 Person pages and 11 hub `canonical:` fields now agree: [[Boskovic|Bošković]], [[Draskovic|Drašković]], [[Galesic|Galešić]], [[Jadresic|Jadrešić]], [[Mestrovic|Meštrović]], [[Misulic|Mišulić]], [[Pilizaric|Piližarić]], [[Piljusic|Piljušić]], [[Pusic|Pušić]], [[Radasinovic|Radašinović]], [[Rasnjevic|Rašnjević]], [[Saric|Šarić]], [[Siljanovic|Šiljanović]], [[Sodic|Šodić]], [[Vicinovic|Vičinović]], [[Visic|Višić]], [[Vranesic|Vranešić]], [[Vuskovic|Vušković]]. Each hub carries a dated `Spelling note`.
- **2 deliberately left split**, because the diacritic is not simply missing: [[Pecic|Pećić]] / Pečić (genuinely different names, ć vs č) and [[Zrinski]] / Zrinški (here the *undotted* form is the correct one — the Zrinski family).
- **2 further conflicts flagged, not decided** — [[Pecanic|Pečanić]] vs Pećanić and [[Sincevic|Sinčević]] vs Šinčević, where the 1756 normalization disagrees with the earlier pages and the sources do not settle it. Both hubs carry an `unresolved` spelling note.
- Filenames were untouched (they are diacritic-stripped), so no links moved; 0 broken links after the pass.
- Also fixed while validating: [[Jure-Bugarin-b????-Skabrnja]] had unparseable YAML frontmatter (`surname_variants: [Bugarija (as printed, with ?)]` — an unquoted `?` and parentheses break a flow sequence), dating from the Škabrnja ingest. Now `["Bugarija"]` with the printed form in a comment. **All 2,543 Person pages and every page in `wiki/` now parse as valid YAML.**
- **Effect on the comparisons:** reconciliation raised measured 1610→1756 surname continuity in three of the four villages — Sukošan 11→12 of 39, Turanj 4→5 of 33, Biograd 6→8 of 51 — a concrete demonstration of the "treat the continuity figures as a lower bound" caveat on those pages. The comparison sections were regenerated and the counts updated in [[index]] and here.

## [2026-09-08] ingest | Backfill — Sv. Filip i Jakov, 1610 census (Dundović)
The village *is* in [[Dundovic-1610-Zadar-Census]] but the 2026-07-21 ingest skipped it; found while building the 1610→1756 comparisons. `raw/transcriptions/dundovic-1610-zadar-census.txt` lines 1321–1347, split across a page break under a repeated heading — the likely reason it was missed.
- **Persons added: 16** household heads, all `confidence: low`, all with `birth_place_inferred: true`.
- **Surnames: 2 new hubs** ([[Galantin]], [[Bakinovic|Bakinović]]), 8 existing gained 1610 bearers ([[Velicic|Veličić]], [[Maletic|Maletić]], [[Kuzinovic|Kužinović]], [[Barbarosa]], [[Milatovic|Milatović]], [[Koskic|Koškić]], [[Turkovic|Turković]], [[Lukacic|Lukačić]]).
- **Arithmetic discrepancy recorded, not resolved:** the printed *Ukupno* row reads 28 men / 36 women / 25 boys / 21 girls = **110**, but the sixteen household rows sum to **102** (27/34/24/17). Every row is internally consistent, so the per-household figures are used and the printed total is flagged on [[Sv-Filip-i-Jakov|the Place page]].
- Four households appear under three spellings — *Cuzenouich*, *Chuxinouich*, *Chuxinoucih* — filed as one [[Kuzinovic|Kužinović]] family and noted on each page. One widow heads a household: *Gierca Velicichia*, the feminine surname form ([[Jerka-Velicic-b????-Sv-Filip-i-Jakov]]).
- [[Sv-Filip-i-Jakov|Sv. Filip i Jakov]] now has a `## 1610 → 1756: what changed` section like the other four villages — **3 of 10** surnames continue, proportionally the highest of the five.

## [2026-09-08] note | Smoljić merged into Smolić; birth_place retrofit
- **[[Smolic|Smolić]] ← Smoljić merged** (user decision). The 1610 census spells the Sukošan name *Smoglich*, which the original ingest normalized to *Smoljić* on its own hub; the same village carries eight Smolić households in 1756. Three Person pages moved (`git mv`, history preserved): Grgur and Ivan took the plain ids, and **Marko** became [[Marko-Smolic-b????-Sukosan-2]] because [[Marko-Smolic-b????-Sukosan]] is a different man in the 1756 survey — cross-noted on both. `Smoglich` added to the hub's variants; `Smoljic.md` deleted.
  - Effect: Sukošan's 1610→1756 continuity rises to **13 of 39**, and Smolić is now visibly the village's largest surviving kin-group (3 households → 8).
- **`birth_place_inferred: true` retrofitted to all 621 remaining [[Dundovic-1610-Zadar-Census]] Person pages**, each with the standard Notes bullet. All 637 pages from that source now follow the CLAUDE.md convention, matching the 1,431 pages from [[Stanojevic-1756-Zadar-Census]].
- **Two more canonical-spelling conflicts found and fixed** — both introduced by the 1756 ingest disagreeing with an existing hub: `Kuzinović` → **Kužinović** and `Barbarossa` → **Barbarosa** (4 Person pages). Both surnames then showed up correctly as continuing in Sv. Filip i Jakov rather than as a disappearance plus an arrival.
- Also tidied: [[Kuzinovic|Kužinović]]'s `canonical` field, which read `Kužinović (Kuzinović)`; and 107 surname hubs that listed their own canonical form as a variant.
- Still unresolved by choice: [[Pecic|Pećić]]/Pečić, [[Zrinski]]/Zrinški, [[Pecanic|Pečanić]]/Pećanić, [[Sincevic|Sinčević]]/Šinčević.
